...
import static com.app.pixstory.data.DataManager.LogInMode.LOGGED_IN_MODE_LOGGED_IN_EMAIL;
...
public class LoginViewModel extends BaseViewModel {
    ...
    public void onLoginWithUsernamePassword(String loginType, String username, String password) {
        isLoading(true);
                                ...
                                if (response.getSuccess()) {
                                    if (response.getData() != null) {
                                        if (response.getData().getFormCompletion() == 1) {
                                            ...
                                            ((LoginNavigator) getNavigator()).openInterest();
                                        } else if (response.getData().getFormCompletion() == 2) {
                                            getDataManager().setAccessToken(response.getData().getToken());
                                            getDataManager().setLoginMode(LOGGED_IN_MODE_LOGGED_IN_EMAIL.getType());
                                            ((LoginNavigator) getNavigator()).openHomeActivity();
                                        }
                                    ...
                                    ((LoginNavigator) getNavigator()).message(response.getMessage());
                                }
                            } else {
                                ((LoginNavigator) getNavigator()).message(R.string.default_error);
                            }
                        },
                        throwable -> {
                            isLoading(false);
                            if (throwable instanceof UnknownHostException) {
                                ((LoginNavigator) getNavigator()).message(R.string.default_network_error);
                            } else {
                                ((LoginNavigator) getNavigator()).message(R.string.default_error);
                            }
    ...
    public void sendEmailOtp(String email, String type, boolean resendOtp) {
        ...
        getCompositeDisposable().add(getDataManager()
                ...
                .subscribe(response -> {
                            ...
                            if (response != null) {
                                if (response.getSuccess()) {
                                    ((LoginNavigator) getNavigator()).onEmailVerificationResponse(response.getSuccess(), email, response.getMessage(), resendOtp, response.getData().getToken());
                                } else {
                                    ((LoginNavigator) getNavigator()).onEmailVerificationResponse(false, email, response.getMessage(), resendOtp, "");
                                }
                            } else {
                                ((LoginNavigator) getNavigator()).onEmailVerificationResponse(false, email, response.getMessage(), resendOtp, "");
                            }
                        ...
                        throwable -> {
                            isLoading(false);
                            ((LoginNavigator) getNavigator()).onEmailVerificationResponse(false, email, null, resendOtp, "");
                            throwable.printStackTrace();
                        })
        );
    }
                            ...
                            if (response != null) {
                                if (response.getSuccess()) {
                                    ...
                                    if (type.equals(VERIFY_TYPE_LOGIN)) {
                                        ...
                                        ((LoginNavigator) getNavigator()).openSignupEmail(email);
    ...
    public void doSocialLogin(String loginType, String facebookId, String linkedIn, String google, HashMap<String, String> data) {
        ...
        getCompositeDisposable().add(getDataManager()
                ...
                .subscribe(response -> {
                            ...
                            if (response != null) {
                                if (response.getSuccess()) {
                                    if (response.getData().getFormCompletion() == 1) {
                                        ...
                                        // getDataManager().setAccessToken(response.getData().getToken());
                                        ((LoginNavigator) getNavigator()).openSignupFromSocial(loginType, data);
                                        // ((LoginNavigator) getNavigator()).message(response.getMessage());
                                    }
                                } else {
                        ...
                        throwable -> {
                            isLoading(false);
                            ((LoginNavigator) getNavigator()).message("Something went wrong.Try again!!!");
                            throwable.printStackTrace();
                        })
        );
    }
    ...
    public void verificationRequestMobile(String countryCode, String mobile, boolean resendOtp, String type) {
        ...
        getCompositeDisposable().add(getDataManager()
                ...
                .subscribe(response -> {
                            ...
                            if (response != null) {
                                if (response.getSuccess()) {
                                    ((LoginNavigator) getNavigator()).onVerificationResponse(response.getSuccess(), countryCode,
                                            mobile, response.getMessage(), resendOtp);
                                } else {
                                    ((LoginNavigator) getNavigator()).onVerificationResponse(false, countryCode,
                                            mobile, response.getMessage(), resendOtp);
                                }
                            } else {
                                ((LoginNavigator) getNavigator()).onVerificationResponse(false, countryCode,
                                ...
                                if (response.getSuccess()) {
                                    if (response.getData() != null && response.getData().getType() != null) {
                                        if (response.getData().getType() == 1) {
                                            ...
                                            getDataManager().setAccessToken(response.getData().getToken());
                                            ((LoginNavigator) getNavigator()).openSignup(countryCode, mobile);
                                        }
                                    }
                                } else {
                                    ((LoginNavigator) getNavigator()).message(response.getMessage());
                            ...
                            if (response != null && response.getStatus()) {
                                ...
                                if (vCode > BuildConfig.VERSION_CODE) {
                                    if (getNavigator() instanceof LoginNavigator) {
                                        ((LoginNavigator) getNavigator()).openAppUpgrade(vName, vCode);
                                    }
                                }
                            }
                        },
